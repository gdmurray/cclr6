...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const pageContent: RuleItem[] = lineData.map((line, index) => {
        if (index === 0) {
            return {
                className: 'title',
                content: line
    ...
    pageContent.filter((item) => item.id !== undefined).forEach((elem) => {
                                    ...
                                    <div key={section.section.id} className='table-block'>
                                        ...
                                        <div className='table-subsections'>
                                            {section.subsections.map((subsection) => {
                                                return (
                                                    <a key={subsection.id}
                                                       href={`#${subsection.id}`}>{subsection.content}</a>
                                                )
                                            })}
                                        </div>
                                    </div>
            ...
            <div className='rulebook text-content'>
                {getTableOfContents()}
                {
                    content.map(({ id, className, content }, index) => {
                        ...
                        return <p id={id} key={key} className={className}>{content}</p>
                    })
                }
