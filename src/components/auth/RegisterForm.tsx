...
import {
    ...
    IconButton,
    Popover,
    PopoverTrigger, PopoverContent, PopoverArrow, PopoverCloseButton, PopoverHeader, PopoverBody, Button
    ...
    const PasswordRequirementsPopover = (): JSX.Element => {
        return (
            <Popover trigger='hover'>
                <PopoverTrigger>
                    <span className='ml-1 cursor-pointer text-alt-2'><FaQuestionCircle /></span>
                </PopoverTrigger>
                <PopoverContent>
                    <PopoverArrow />
                    <PopoverCloseButton />
                    ...
                    <PopoverBody>
                        <div className='space-y-0.5'>
                            {Object.keys(passwordChecklist).map((check) => {
                                ...
                                return (
                                    <div key={check} className='flex flex-row'>{value ? (
                                        <FaCheck className='text-success mt-0.5 mr-1 ' />) : (
                                        <FaTimes className='text-error mt-0.5 mr-0.5' />)} {check}
                                    </div>)
                            })}
                        </div>
                    </PopoverBody>
            ...
            <div
                ...
                <form className='w-full max-w-xl mb-8 py-4' onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <FormControl
                        ...
                        isRequired
                    >
                        <FormLabel style={{ display: 'flex' }} className='flex flex-row items-center'>Password&nbsp;
                            <PasswordRequirementsPopover /></FormLabel>
                        <InputGroup>
